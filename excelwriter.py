...
def generate_excel(database, filename):
    ...
    for i in range(tx_count):
        ...
        tx_name = database[i+1][0].rsplit('/')[-1]
        title_name = "t" + str(i+1) + "\n" + tx_name
        sheet.write_merge(0, 1, 3*i, 3*i+2, title_name, title_style)
    ...
    sim_name = filename.split('/')[-2]
    workbook.save(filename+'/' + sim_name + ".xls")
